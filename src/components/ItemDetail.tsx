import { ArrowLeft, Share2, MapPin, Clock, Star, RefreshCw } from "lucide-react";
            ...
            <CardContent className="p-4">
              ...
              {isOwner ? (
                <div className="space-y-2">
                  ...
                  {/* If there are accepted requests, provide a Complete button */}
                  {exchangeRequests.filter(req => req.status === 'accepted').map((acceptedReq) => (
                    <div key={acceptedReq.id} className="p-3 border rounded-lg bg-green-50">
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-medium">{acceptedReq.requesterName} — Accepted</p>
                        {acceptedReq.ownerConfirmed || acceptedReq.requesterConfirmed ? (
                          <div className="text-sm text-muted-foreground">Waiting for the other party</div>
                        ) : (
                          <Button size="sm" onClick={() => onCompleteExchange?.(acceptedReq.id)}>
                            Mark Completed
                          </Button>
                        )}
                      </div>
